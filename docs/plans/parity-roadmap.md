...
- [~] engine integration of `graph/` — mostly done (ADR 0009/0010 + addendum):
  ...
  interface as trailing signal inputs; smooth per-note parameter ramps
  shipped: a steal/retrigger glides `p1`..`p4` linearly from the stolen
  note's current values to the new note's over a per-program window
