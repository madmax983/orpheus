...
mod tests {
    ...
    fn should_calculate_delay_frames_correctly() {
        let time = Rational::new(1, 2).expect("valid rational");
    ...
    fn should_return_error_when_time_is_zero() {
        let time = Rational::new(0, 1).expect("valid rational");
        let frames_per_cycle = 44100;
        assert!(matches!(
    ...
    fn should_initialize_delay_state_correctly() {
        let spec = DelaySpec::new(Rational::new(1, 4).expect("valid rational"), 0.5, 0.2);
    ...
    fn should_sync_timing_correctly() {
        let spec1 = DelaySpec::new(Rational::new(1, 4).expect("valid rational"), 0.5, 0.2);
        let mut state = DelayState::new(&spec1, 44100).unwrap();

        let spec2 = DelaySpec::new(Rational::new(1, 2).expect("valid rational"), 0.7, 0.3);
    ...
    fn should_process_frame_correctly() {
        let spec = DelaySpec::new(Rational::new(1, 44100).expect("valid rational"), 0.5, 1.0); // 1 frame delay
