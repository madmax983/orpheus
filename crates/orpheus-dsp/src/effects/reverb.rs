...
mod tests {
    ...
    fn should_process_comb_filter() {
        let mut comb = CombState::new(4, 0.5, 0.0);
        ...
        assert_eq!(comb.buffer[0], 0.5); // input (0) + feedback(0.5) * prev_out(1.0)
        assert_eq!(comb.index, 1);
    }
    ...
    fn should_process_allpass_filter() {
        ...
        assert_eq!(allpass.buffer[0], 1.0); // 1.0 + 0.5 * 0.0
        ...
        assert_eq!(allpass.buffer[0], 0.5);
    }
}
