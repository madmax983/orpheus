...
impl EngineCore {
    ...
    fn mix_plugin_sources(&mut self, local_frame: u64) {
        for track in self.active_routing.tracks() {
    ...
    fn sum_tracks_to_buses_and_master(&mut self) -> (f32, f32) {
    ...
    fn mix_routed_voices(&mut self) -> (f32, f32) {
        ...
        self.mix_plugin_sources(local_frame);

        let (mut master_left, mut master_right) = self.sum_tracks_to_buses_and_master();
