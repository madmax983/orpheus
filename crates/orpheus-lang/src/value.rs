    ...
    T: PatternRuntimeValue,
{
    ...
    fn try_query_transform_method(&self, span: &TimeSpan) -> Result<Vec<Event<T>>, EvalError> {
        if let Some(events) = self.try_query_transform_method_core(span)? {
            return Ok(events);
        }
        if let Some(events) = self.try_query_transform_method_pitch(span)? {
            return Ok(events);
        }
        if let Some(events) = self.try_query_transform_method_time(span)? {
            return Ok(events);
        }
        self.try_query_transform_method_other(span)
    }

    fn try_query_transform_method_core(
        &self,
        span: &TimeSpan,
    ...
    fn try_query_transform_method_pitch(
        &self,
        span: &TimeSpan,
    ) -> Result<Option<Vec<Event<T>>>, EvalError> {
        let res = match self {
    ...
    fn try_query_transform_method_time(
        &self,
        span: &TimeSpan,
    ) -> Result<Option<Vec<Event<T>>>, EvalError> {
        let res = match self {
    ...
    fn try_query_transform_method_other(
        &self,
        span: &TimeSpan,
    ...
    fn try_query_audio_effect_method(&self, span: &TimeSpan) -> Result<Vec<Event<T>>, EvalError> {
        if let Some(events) = self.try_query_audio_effect_method_delay(span)? {
            return Ok(events);
        }
        if let Some(events) = self.try_query_audio_effect_method_reverb(span)? {
            return Ok(events);
        }
        self.try_query_audio_effect_method_filter(span)
    }

    fn try_query_audio_effect_method_delay(
        &self,
        span: &TimeSpan,
    ) -> Result<Option<Vec<Event<T>>>, EvalError> {
        let res = match self {
    ...
    fn try_query_audio_effect_method_reverb(
        &self,
        span: &TimeSpan,
    ) -> Result<Option<Vec<Event<T>>>, EvalError> {
        let res = match self {
    ...
    fn try_query_audio_effect_method_filter(
        &self,
        span: &TimeSpan,
    ) -> Result<Vec<Event<T>>, EvalError> {
        match self {
            Self::Hpf { cutoff_hz, inner } => {
                apply_value_mutation(inner, span, |value| *value = value.adjust_hpf(*cutoff_hz))
            }
            Self::HpfPattern { control, inner } => {
                apply_control_pattern(inner, control, span, ControlPatternKind::Hpf)
            }
            Self::Lpf { cutoff_hz, inner } => {
                apply_value_mutation(inner, span, |value| *value = value.adjust_lpf(*cutoff_hz))
            }
            ...
            Self::Drive { drive, inner } => {
                apply_value_mutation(inner, span, |value| *value = value.adjust_drive(*drive))
            }
    ...
    fn try_query_modulation_effect_method(
        ...
        if let Some(events) = self.try_query_modulation_effect_method_chorus(span)? {
            return Ok(events);
        }
        if let Some(events) = self.try_query_modulation_effect_method_compressor(span)? {
            return Ok(events);
        }
        self.try_query_modulation_effect_method_other(span)
    }

    fn try_query_modulation_effect_method_chorus(
        &self,
        span: &TimeSpan,
    ) -> Result<Option<Vec<Event<T>>>, EvalError> {
        let res = match self {
    ...
    fn try_query_modulation_effect_method_compressor(
        &self,
        span: &TimeSpan,
    ) -> Result<Option<Vec<Event<T>>>, EvalError> {
        let res = match self {
    ...
    fn try_query_modulation_effect_method_other(
        &self,
        span: &TimeSpan,
    ) -> Result<Vec<Event<T>>, EvalError> {
        match self {
