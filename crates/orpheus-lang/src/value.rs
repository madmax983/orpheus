    ...
    T: PatternRuntimeValue,
{
    ...
    fn try_query_transform_method(&self, span: &TimeSpan) -> Result<Vec<Event<T>>, EvalError> {
        if let Some(events) = self.try_query_structural_transform(span)? {
            return Ok(events);
        }
        if let Some(events) = self.try_query_pitch_transform(span)? {
            return Ok(events);
        }
        if let Some(events) = self.try_query_time_transform(span)? {
            return Ok(events);
        }
        if let Some(events) = self.try_query_other_transform(span)? {
            return Ok(events);
        }
        Err(EvalError::new("not a valid transform pattern"))
    }

    fn try_query_structural_transform(
        &self,
        span: &TimeSpan,
    ) -> Result<Option<Vec<Event<T>>>, EvalError> {
        match self {
            Self::Roll { steps, inner } => {
                Ok(Some(T::roll_events(inner.try_query(span)?, *steps)?))
            }
            Self::Strum { inner } => Ok(Some(T::strum_events(inner.try_query(span)?)?)),
            Self::Arp {
                steps,
                direction,
                inner,
            } => Ok(Some(T::arp_events(
                inner.try_query(span)?,
                *steps,
                *direction,
            )?)),
            Self::Invert { count, inner } => {
                Ok(Some(T::invert_events(inner.try_query(span)?, *count)?))
            }
            Self::Drop { count, inner } => {
                Ok(Some(T::drop_events(inner.try_query(span)?, *count)?))
            }
            _ => Ok(None),
        }
    }

    fn try_query_pitch_transform(
        &self,
        span: &TimeSpan,
    ) -> Result<Option<Vec<Event<T>>>, EvalError> {
        match self {
            Self::Degrees { collection, inner } => {
                Ok(Some(apply_value_transform(inner, span, |value| {
                    value.map_degrees(collection)
                })?))
            }
            Self::Transpose { semitones, inner } => {
                Ok(Some(apply_value_transform(inner, span, |value| {
                    value.transpose_semitones(*semitones)
                })?))
            }
            Self::TransposePattern { control, inner } => Ok(Some(apply_control_pattern(
                inner,
                control,
                span,
            ...
            Self::Pitch { semitones, inner } => {
                Ok(Some(apply_value_mutation(inner, span, |value| {
                    *value = value.adjust_rate(semitones_to_rate_multiplier(*semitones));
                })?))
            }
            Self::PitchPattern { control, inner } => Ok(Some(apply_control_pattern(
                inner,
                control,
                span,
            ...
            Self::TunedPitchPattern {
                ...
                tuning,
                inner,
            } => Ok(Some(apply_tuned_pitch_pattern(
                inner, control, span, tuning,
            )?)),
            _ => Ok(None),
        }
    }

    fn try_query_time_transform(
        &self,
        span: &TimeSpan,
    ) -> Result<Option<Vec<Event<T>>>, EvalError> {
        match self {
            Self::Fast { factor, inner } => Ok(Some(query_fast(inner, *factor, span)?)),
            Self::Slow { factor, inner } => Ok(Some(query_slow(inner, *factor, span)?)),
            Self::Shift { offset, inner } => Ok(Some(query_shift(inner, offset, span)?)),
            Self::Rev { inner } => Ok(Some(query_rev(inner, span)?)),
            _ => Ok(None),
        }
    }

    fn try_query_other_transform(
        &self,
        span: &TimeSpan,
    ) -> Result<Option<Vec<Event<T>>>, EvalError> {
        match self {
            Self::Gain { factor, inner } => Ok(Some(apply_value_mutation(inner, span, |value| {
                *value = value.adjust_gain(*factor);
            })?)),
            Self::GainPattern { control, inner } => Ok(Some(apply_control_pattern(
                inner,
                control,
                span,
                ControlPatternKind::Gain,
            )?)),
            Self::Rate { factor, inner } => Ok(Some(apply_value_mutation(inner, span, |value| {
                *value = value.adjust_rate(*factor);
            })?)),
            Self::RatePattern { control, inner } => Ok(Some(apply_control_pattern(
                inner,
                control,
                span,
                ControlPatternKind::Rate,
            )?)),
            Self::Onset { onset_index, inner } => {
                Ok(Some(apply_value_mutation(inner, span, |value| {
                    *value = value.adjust_onset(*onset_index);
                })?))
            }
            Self::OnsetPattern { control, inner } => {
                Ok(Some(apply_onset_pattern(inner, control, span)?))
            }
            ...
            Self::SliceIdxPattern {
                ...
                segments,
                inner,
            } => Ok(Some(apply_slice_idx_pattern(
                inner, control, *segments, span,
            )?)),
            Self::Pedal {
                pedal_program,
                inner,
            ...
            Self::Rand { site_salt } => Ok(Some(query_rand(*site_salt, span)?)),
            _ => Ok(Some(self.try_query_audio_effect(span)?)),
        }
    }
    ...
    fn try_query_audio_effect_method(&self, span: &TimeSpan) -> Result<Vec<Event<T>>, EvalError> {
        if let Some(events) = self.try_query_delay_effect(span)? {
            return Ok(events);
        }
        if let Some(events) = self.try_query_filter_effect(span)? {
            return Ok(events);
        }
        if let Some(events) = self.try_query_reverb_effect(span)? {
            return Ok(events);
        }
        if let Some(events) = self.try_query_other_audio_effect(span)? {
            return Ok(events);
        }
        Err(EvalError::new("not a valid audio effect"))
    }

    fn try_query_delay_effect(&self, span: &TimeSpan) -> Result<Option<Vec<Event<T>>>, EvalError> {
        match self {
            Self::Delay { mix, inner } => Ok(Some(apply_value_mutation(inner, span, |value| {
                *value = value.adjust_delay_mix(*mix);
            })?)),
            Self::DelayPattern { control, inner } => Ok(Some(apply_control_pattern(
                inner,
                control,
                span,
            ...
            Self::DelayTime { time, inner } => {
                Ok(Some(apply_value_mutation(inner, span, |value| {
                    *value = value.adjust_delay_time(*time);
                })?))
            }
            Self::DelayTimePattern { control, inner } => Ok(Some(apply_control_pattern(
                inner,
                control,
                span,
            ...
            Self::DelayFeedbackPattern { control, inner } => Ok(Some(apply_control_pattern(
                inner,
                control,
                span,
    ...
    fn try_query_filter_effect(&self, span: &TimeSpan) -> Result<Option<Vec<Event<T>>>, EvalError> {
        match self {
            ...
            Self::HpfPattern { control, inner } => Ok(Some(apply_control_pattern(
                inner,
                control,
                span,
            ...
            Self::LpfPattern { control, inner } => Ok(Some(apply_control_pattern(
                inner,
                control,
                span,
    ...
    fn try_query_reverb_effect(&self, span: &TimeSpan) -> Result<Option<Vec<Event<T>>>, EvalError> {
        match self {
            Self::Reverb { mix, inner } => Ok(Some(apply_value_mutation(inner, span, |value| {
                *value = value.adjust_reverb_mix(*mix);
            })?)),
            Self::ReverbPattern { control, inner } => Ok(Some(apply_control_pattern(
                inner,
                control,
                span,
                ControlPatternKind::ReverbMix,
            )?)),
            Self::ReverbRoom { room, inner } => {
                Ok(Some(apply_value_mutation(inner, span, |value| {
                    *value = value.adjust_reverb_room(*room);
                })?))
            }
            Self::ReverbRoomPattern { control, inner } => Ok(Some(apply_control_pattern(
                inner,
                control,
                span,
                ControlPatternKind::ReverbRoom,
            )?)),
            Self::ReverbDamp { damp, inner } => {
                Ok(Some(apply_value_mutation(inner, span, |value| {
                    *value = value.adjust_reverb_damp(*damp);
                })?))
            }
            Self::ReverbDampPattern { control, inner } => Ok(Some(apply_control_pattern(
                inner,
                control,
                span,
    ...
    fn try_query_other_audio_effect(
        &self,
        span: &TimeSpan,
    ) -> Result<Option<Vec<Event<T>>>, EvalError> {
        match self {
            Self::Res { resonance, inner } => {
                Ok(Some(apply_value_mutation(inner, span, |value| {
                    *value = value.adjust_resonance(*resonance);
                })?))
            }
            Self::ResPattern { control, inner } => Ok(Some(apply_control_pattern(
                inner,
                control,
                span,
                ControlPatternKind::Res,
            )?)),
            Self::Drive { drive, inner } => Ok(Some(apply_value_mutation(inner, span, |value| {
                *value = value.adjust_drive(*drive);
            })?)),
            Self::DrivePattern { control, inner } => Ok(Some(apply_control_pattern(
                inner,
                control,
                span,
                ControlPatternKind::Drive,
            )?)),
            _ => Ok(Some(self.try_query_modulation_effect(span)?)),
        }
    }
    ...
    fn try_query_modulation_effect_method(
        ...
        if let Some(events) = self.try_query_chorus_effect(span)? {
            return Ok(events);
        }
        if let Some(events) = self.try_query_compressor_effect(span)? {
            return Ok(events);
        }
        if let Some(events) = self.try_query_other_modulation_effect(span)? {
            return Ok(events);
        }
        unreachable!("handled in previous try_query stages")
    }

    fn try_query_chorus_effect(&self, span: &TimeSpan) -> Result<Option<Vec<Event<T>>>, EvalError> {
        match self {
            Self::Chorus { mix, inner } => Ok(Some(apply_value_mutation(inner, span, |value| {
                *value = value.adjust_chorus_mix(*mix);
            })?)),
            Self::ChorusPattern { control, inner } => Ok(Some(apply_control_pattern(
                inner,
                control,
                span,
                ControlPatternKind::ChorusMix,
            )?)),
            Self::ChorusDepth { depth, inner } => {
                Ok(Some(apply_value_mutation(inner, span, |value| {
                    *value = value.adjust_chorus_depth(*depth);
                })?))
            }
            Self::ChorusDepthPattern { control, inner } => Ok(Some(apply_control_pattern(
                inner,
                control,
                span,
                ControlPatternKind::ChorusDepth,
            )?)),
            Self::ChorusRate { rate, inner } => {
                Ok(Some(apply_value_mutation(inner, span, |value| {
                    *value = value.adjust_chorus_rate(*rate);
                })?))
            }
            Self::ChorusRatePattern { control, inner } => Ok(Some(apply_control_pattern(
                inner,
                control,
                span,
    ...
    fn try_query_compressor_effect(
        &self,
        span: &TimeSpan,
    ) -> Result<Option<Vec<Event<T>>>, EvalError> {
        match self {
            Self::Compressor { mix, inner } => {
                Ok(Some(apply_value_mutation(inner, span, |value| {
                    *value = value.adjust_compressor_mix(*mix);
                })?))
            }
            Self::CompressorPattern { control, inner } => Ok(Some(apply_control_pattern(
                inner,
                control,
                span,
            ...
            Self::CompressorThresholdPattern { control, inner } => Ok(Some(apply_control_pattern(
                inner,
                control,
                span,
                ControlPatternKind::CompressorThreshold,
            )?)),
            Self::CompressorRatio { ratio, inner } => {
                Ok(Some(apply_value_mutation(inner, span, |value| {
                    *value = value.adjust_compressor_ratio(*ratio);
                })?))
            }
            Self::CompressorRatioPattern { control, inner } => Ok(Some(apply_control_pattern(
                inner,
                control,
                span,
    ...
    fn try_query_other_modulation_effect(
        &self,
        span: &TimeSpan,
    ) -> Result<Option<Vec<Event<T>>>, EvalError> {
        match self {
            ...
            Self::PulseWidthPattern { control, inner } => Ok(Some(apply_control_pattern(
                inner,
                control,
                span,
                ControlPatternKind::PulseWidth,
            )?)),
            Self::Pan { amount, inner } => Ok(Some(apply_value_mutation(inner, span, |value| {
                *value = value.adjust_pan(*amount);
            })?)),
            Self::PanPattern { control, inner } => Ok(Some(apply_control_pattern(
                inner,
                control,
                span,
