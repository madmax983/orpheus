...
/// ⚡ Bolt: `PatternValueTransform` methods now take `&mut self` and perform in-place mutations.
/// Previously, these methods took `&self` and returned `Self` by allocating a full clone of `SampleEvent`
/// on every FX/modifier application during pattern evaluation. This change eliminates thousands of intermediate
/// heap allocations per frame by mutating the working event directly.
    ...
    apply_event_fragments(
        ...
        |part, value| {
            ...
            Ok(Some({
                ...
                v.adjust_slice(relative_start, relative_end);
                v
            }))
        },
    )
