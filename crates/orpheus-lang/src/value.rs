...
impl<T> PatternRuntime<T> {
    pub(crate) fn map_inner(self, mut f: impl FnMut(Self) -> Self) -> Self {
        match self {
            Self::Every { period, transform, inner } => Self::Every { period, transform, inner: Box::new(f(*inner)) },
            Self::When { period, offset, transform, inner } => Self::When { period, offset, transform, inner: Box::new(f(*inner)) },
            Self::Sometimes { site_salt, transform, inner } => Self::Sometimes { site_salt, transform, inner: Box::new(f(*inner)) },
            Self::Within { start, end, transform, inner } => Self::Within { start, end, transform, inner: Box::new(f(*inner)) },
            Self::Mask { gate, inner } => Self::Mask { gate, inner: Box::new(f(*inner)) },
            Self::Strum { inner } => Self::Strum { inner: Box::new(f(*inner)) },
            Self::Roll { steps, inner } => Self::Roll { steps, inner: Box::new(f(*inner)) },
            Self::Arp { steps, direction, inner } => Self::Arp { steps, direction, inner: Box::new(f(*inner)) },
            Self::Invert { count, inner } => Self::Invert { count, inner: Box::new(f(*inner)) },
            Self::Drop { count, inner } => Self::Drop { count, inner: Box::new(f(*inner)) },
            Self::Degrees { collection, inner } => Self::Degrees { collection, inner: Box::new(f(*inner)) },
            Self::Transpose { semitones, inner } => Self::Transpose { semitones, inner: Box::new(f(*inner)) },
            Self::TransposePattern { control, inner } => Self::TransposePattern { control, inner: Box::new(f(*inner)) },
            Self::Fast { factor, inner } => Self::Fast { factor, inner: Box::new(f(*inner)) },
            Self::Slow { factor, inner } => Self::Slow { factor, inner: Box::new(f(*inner)) },
            Self::Shift { offset, inner } => Self::Shift { offset, inner: Box::new(f(*inner)) },
            Self::Rev { inner } => Self::Rev { inner: Box::new(f(*inner)) },
            Self::Chaos { site_salt, inner } => Self::Chaos { site_salt, inner: Box::new(f(*inner)) },
            Self::Gain { factor, inner } => Self::Gain { factor, inner: Box::new(f(*inner)) },
            Self::GainPattern { control, inner } => Self::GainPattern { control, inner: Box::new(f(*inner)) },
            Self::Delay { mix, inner } => Self::Delay { mix, inner: Box::new(f(*inner)) },
            Self::DelayPattern { control, inner } => Self::DelayPattern { control, inner: Box::new(f(*inner)) },
            Self::DelayTime { time, inner } => Self::DelayTime { time, inner: Box::new(f(*inner)) },
            Self::DelayTimePattern { control, inner } => Self::DelayTimePattern { control, inner: Box::new(f(*inner)) },
            Self::DelayFeedback { feedback, inner } => Self::DelayFeedback { feedback, inner: Box::new(f(*inner)) },
            Self::DelayFeedbackPattern { control, inner } => Self::DelayFeedbackPattern { control, inner: Box::new(f(*inner)) },
            Self::Hpf { cutoff_hz, inner } => Self::Hpf { cutoff_hz, inner: Box::new(f(*inner)) },
            Self::HpfPattern { control, inner } => Self::HpfPattern { control, inner: Box::new(f(*inner)) },
            Self::Lpf { cutoff_hz, inner } => Self::Lpf { cutoff_hz, inner: Box::new(f(*inner)) },
            Self::LpfPattern { control, inner } => Self::LpfPattern { control, inner: Box::new(f(*inner)) },
            Self::Reverb { mix, inner } => Self::Reverb { mix, inner: Box::new(f(*inner)) },
            Self::ReverbPattern { control, inner } => Self::ReverbPattern { control, inner: Box::new(f(*inner)) },
            Self::ReverbRoom { room, inner } => Self::ReverbRoom { room, inner: Box::new(f(*inner)) },
            Self::ReverbRoomPattern { control, inner } => Self::ReverbRoomPattern { control, inner: Box::new(f(*inner)) },
            Self::ReverbDamp { damp, inner } => Self::ReverbDamp { damp, inner: Box::new(f(*inner)) },
            Self::ReverbDampPattern { control, inner } => Self::ReverbDampPattern { control, inner: Box::new(f(*inner)) },
            Self::Res { resonance, inner } => Self::Res { resonance, inner: Box::new(f(*inner)) },
            Self::ResPattern { control, inner } => Self::ResPattern { control, inner: Box::new(f(*inner)) },
            Self::Drive { drive, inner } => Self::Drive { drive, inner: Box::new(f(*inner)) },
            Self::DrivePattern { control, inner } => Self::DrivePattern { control, inner: Box::new(f(*inner)) },
            Self::Chorus { mix, inner } => Self::Chorus { mix, inner: Box::new(f(*inner)) },
            Self::ChorusPattern { control, inner } => Self::ChorusPattern { control, inner: Box::new(f(*inner)) },
            Self::ChorusDepth { depth, inner } => Self::ChorusDepth { depth, inner: Box::new(f(*inner)) },
            Self::ChorusDepthPattern { control, inner } => Self::ChorusDepthPattern { control, inner: Box::new(f(*inner)) },
            Self::ChorusRate { rate, inner } => Self::ChorusRate { rate, inner: Box::new(f(*inner)) },
            Self::ChorusRatePattern { control, inner } => Self::ChorusRatePattern { control, inner: Box::new(f(*inner)) },
            Self::PulseWidth { pulse_width, inner } => Self::PulseWidth { pulse_width, inner: Box::new(f(*inner)) },
            Self::PulseWidthPattern { control, inner } => Self::PulseWidthPattern { control, inner: Box::new(f(*inner)) },
            Self::Pan { amount, inner } => Self::Pan { amount, inner: Box::new(f(*inner)) },
            Self::PanPattern { control, inner } => Self::PanPattern { control, inner: Box::new(f(*inner)) },
            Self::Compressor { mix, inner } => Self::Compressor { mix, inner: Box::new(f(*inner)) },
            Self::CompressorPattern { control, inner } => Self::CompressorPattern { control, inner: Box::new(f(*inner)) },
            Self::CompressorThreshold { threshold, inner } => Self::CompressorThreshold { threshold, inner: Box::new(f(*inner)) },
            Self::CompressorThresholdPattern { control, inner } => Self::CompressorThresholdPattern { control, inner: Box::new(f(*inner)) },
            Self::CompressorRatio { ratio, inner } => Self::CompressorRatio { ratio, inner: Box::new(f(*inner)) },
            Self::CompressorRatioPattern { control, inner } => Self::CompressorRatioPattern { control, inner: Box::new(f(*inner)) },
            Self::Pitch { semitones, inner } => Self::Pitch { semitones, inner: Box::new(f(*inner)) },
            Self::PitchPattern { control, inner } => Self::PitchPattern { control, inner: Box::new(f(*inner)) },
            Self::TunedPitch { semitones, tuning, inner } => Self::TunedPitch { semitones, tuning, inner: Box::new(f(*inner)) },
            Self::TunedPitchPattern { control, tuning, inner } => Self::TunedPitchPattern { control, tuning, inner: Box::new(f(*inner)) },
            Self::Rate { factor, inner } => Self::Rate { factor, inner: Box::new(f(*inner)) },
            Self::RatePattern { control, inner } => Self::RatePattern { control, inner: Box::new(f(*inner)) },
            Self::Onset { onset_index, inner } => Self::Onset { onset_index, inner: Box::new(f(*inner)) },
            Self::OnsetPattern { control, inner } => Self::OnsetPattern { control, inner: Box::new(f(*inner)) },
            Self::Slice { start, end, inner } => Self::Slice { start, end, inner: Box::new(f(*inner)) },
            Self::SlicePattern { start_control, end_control, inner } => Self::SlicePattern { start_control, end_control, inner: Box::new(f(*inner)) },
            Self::SliceIdxPattern { control, segments, inner } => Self::SliceIdxPattern { control, segments, inner: Box::new(f(*inner)) },
            Self::Pedal { pedal_program, inner } => Self::Pedal { pedal_program, inner: Box::new(f(*inner)) },
