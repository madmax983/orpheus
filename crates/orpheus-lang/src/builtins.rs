...
fn apply_morse(args: Vec<Value>) -> Result<Value, EvalError> {
    ...
    for (is_tone, length) in symbols_and_gaps {
        let dur = orpheus_pattern::Rational::new(i64::from(length), i64::from(total_units))
...
mod hex_bin_error_tests {
    ...
    fn test_morse_builtin() {
        ...
        let span = orpheus_pattern::TimeSpan::new(
            orpheus_pattern::Rational::zero(),
            orpheus_pattern::Rational::one(),
        )
        .unwrap();
        let events = pattern.try_query(&span).unwrap();

        // "SOS" -> "... --- ..." -> 3 dots, 3 dashes, 3 dots.
        // Dot = 1, Dash = 3, intra-char rest = 1, inter-char rest = 3.
        // 'S' (...) = 1, rest, 1, rest, 1. (Length 5 atoms)
        // inter-char rest = 3.
        // 'O' (---) = 3, rest, 3, rest, 3. (Length 11 atoms)
        // inter-char rest = 3.
        // 'S' (...) = 1, rest, 1, rest, 1. (Length 5 atoms)
        // Total sequence length = 5 + 3 + 11 + 3 + 5 = 27 atoms.
        // Number of non-rest atoms (events) = 3 + 3 + 3 = 9 events.
        assert_eq!(events.len(), 9);
        for event in events {
            assert!((event.value - 1.0).abs() < f64::EPSILON);
        }
    }
