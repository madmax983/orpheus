...
impl ReplSession {
    ...
    fn explain_binding(&self, args: &str) -> Result<String, String> {
        ...
        match value {
            ...
            crate::value::Value::PitchClassSet(pcs) => Ok(pcs.explain(binding_name)),
            crate::value::Value::ArpDirection(arp) => Ok(arp.explain(binding_name)),
            crate::value::Value::PluginPattern(plugin) => Ok(plugin.explain(binding_name)),
            crate::value::Value::String(s) => {
                use crossterm::style::Stylize;
                Ok(format!("{} {}\nValue: {}", "String:".cyan().bold(), binding_name.yellow(), s))
            }
        }
    }
