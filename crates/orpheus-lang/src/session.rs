...
mod tests {
    ...
    #[test]
    fn session_explain_explains_non_pedal_bindings() {
        let mut session = ReplSession::new();
        session.eval_line("drums = bd sn").unwrap();

        let result = session.eval_line(":explain drums").unwrap();

        assert!(result.contains("Sample Pattern Plan:"));
        assert!(result.contains("drums"));
    }
