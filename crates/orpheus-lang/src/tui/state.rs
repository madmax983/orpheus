...
impl SharedState {
    ...
    pub fn with_startup(
        ...
        let mut transcript = vec![
            TranscriptEntry::Plain("Interactive shell ready.".to_owned()),
            TranscriptEntry::Plain("Press Esc to quit (layout mode).".to_owned()),
        ];
        ...
        match self.session.eval_line(&line) {
            Ok(message) => {
                for part in message.lines() {
                    if !part.is_empty() {
                        self.transcript
                            .push(TranscriptEntry::Success(part.to_owned()));
                    }
                }
            }
            Err(message) => {
                for part in message.lines() {
                    if !part.is_empty() {
                        self.transcript
                            .push(TranscriptEntry::Error(part.to_owned()));
                    }
                }
            }
        }
