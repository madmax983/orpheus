...
impl TypeEnv {
    ...
    pub fn with_builtins() -> Self {
        ...
        install_basic_transforms(&mut env, alpha);
        install_probabilistic_builtins(&mut env, alpha);
        install_euclidean_and_counting_builtins(&mut env, alpha);
        install_music_theory_builtins(&mut env);
        install_plugin_builtins(&mut env);
        install_effect_controls(&mut env);
        install_sample_operations(&mut env);
...
fn install_basic_transforms(env: &mut TypeEnv, alpha: TypeVarId) {
...
fn install_music_theory_builtins(env: &mut TypeEnv) {
    env.insert(
        "pitch_class_set",
...
fn install_effect_controls(env: &mut TypeEnv) {
    for name in [
...
fn install_sample_operations(env: &mut TypeEnv) {
    env.insert(
        "sample",
        TypeScheme::monomorphic(Type::curried(
            vec![Type::String],
