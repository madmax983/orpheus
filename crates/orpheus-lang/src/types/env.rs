...
impl TypeEnv {
    ...
    pub fn with_builtins() -> Self {
        ...
        install_base_sample_builtins(&mut env);
        install_cycle_alternation_builtins(&mut env, alpha);
        install_temporal_and_pitch_builtins(&mut env, alpha);
        install_probabilistic_builtins(&mut env, alpha);
        install_euclidean_and_counting_builtins(&mut env, alpha);
        install_scale_and_tuning_builtins(&mut env);
        install_plugin_builtins(&mut env);
        install_effect_and_utility_builtins(&mut env, alpha);

        env
    }
}

fn install_base_sample_builtins(env: &mut TypeEnv) {
    for name in ["bd", "sn", "cp", "hh", "saw", "pulse", "tri", "noise"] {
        env.insert(name, TypeScheme::monomorphic(Type::pattern(Type::Sample)));
    }
}

fn install_temporal_and_pitch_builtins(env: &mut TypeEnv, alpha: TypeVarId) {
...
fn install_scale_and_tuning_builtins(env: &mut TypeEnv) {
    env.insert(
        "pitch_class_set",
...
fn install_effect_and_utility_builtins(env: &mut TypeEnv, alpha: TypeVarId) {
