...
impl TypeEnv {
    ...
    pub fn with_builtins() -> Self {
        ...
        install_pattern_transform_builtins(&mut env, alpha);
        install_sample_control_builtins(&mut env);
        install_misc_builtins(&mut env);
...
fn install_pattern_transform_builtins(env: &mut TypeEnv, alpha: TypeVarId) {
...
fn install_misc_builtins(env: &mut TypeEnv) {
    env.insert(
        "sample",
        TypeScheme::monomorphic(Type::curried(
            vec![Type::String],
