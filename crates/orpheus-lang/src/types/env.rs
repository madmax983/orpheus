...
impl TypeEnv {
    ...
    pub fn with_builtins() -> Self {
        ...
        install_sample_builtins(&mut env);
        install_cycle_alternation_builtins(&mut env, alpha);
        install_numeric_transform_builtins(&mut env, alpha);
        install_probabilistic_builtins(&mut env, alpha);
        install_pitch_and_tuning_builtins(&mut env, alpha);
        ...
        install_misc_builtins(&mut env, alpha);
...
fn install_sample_builtins(env: &mut TypeEnv) {
    for name in ["bd", "sn", "cp", "hh", "saw", "pulse", "tri", "noise"] {
        env.insert(name, TypeScheme::monomorphic(Type::pattern(Type::Sample)));
    }
}

fn install_numeric_transform_builtins(env: &mut TypeEnv, alpha: TypeVarId) {
...
fn install_pitch_and_tuning_builtins(env: &mut TypeEnv, alpha: TypeVarId) {
...
fn install_misc_builtins(env: &mut TypeEnv, alpha: TypeVarId) {
