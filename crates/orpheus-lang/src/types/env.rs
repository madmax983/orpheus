...
impl TypeScheme {
    /// Creates a new monomorphic `TypeScheme` with no type variables.
    ///
    /// Monomorphic types are rigid—they do not adapt to their context.
    /// We use this for basic primitive types like `Sample` or `Number` where
    /// generic polymorphism is unnecessary, avoiding the overhead of unification
...
impl TypeEnv {
    ...
    /// Yields all currently loaded type schemes.
