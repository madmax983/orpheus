...
//!
//! This exporter generates a `.ck` `ChucK` script for evaluated Orpheus patterns.
//! It uses `SndBuf` for sample patterns and `SinOsc` for number patterns,
//! mapping Orpheus scheduling into `ChucK`'s strongly-timed execution model.
...
pub fn export_sample_pattern_to_chuck(
    ...
    for event in events {
        ...
        // In ChucK, we would normally load the file. For this export, we assume the user
        // will provide the files or we just print the intention.
        writeln!(file, "\"{sample}.wav\" => buf.read;")?;
...
///
/// Number patterns are assumed to represent pitch, mapped to a `SinOsc`.
///
/// # Examples
///
/// ```
...
pub fn export_number_pattern_to_chuck(
    ...
    writeln!(file, "0.0 => osc.gain;")?; // start silenced
    writeln!(file)?;
...
mod tests {
    ...
    fn chuck_exporter_generates_sample_pattern() {
        ...
        let path = std::env::temp_dir().join(format!("test_chuck_sample_{}.ck", unique_temp_suffix()));
    ...
    fn chuck_exporter_generates_number_pattern() {
        ...
        let path = std::env::temp_dir().join(format!("test_chuck_num_{}.ck", unique_temp_suffix()));
