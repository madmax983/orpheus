...
//!
//! This exporter generates a `.ck` script containing sporked shred functions
//! for evaluated Orpheus patterns, mapping Orpheus sample identifiers to
//! standard `SndBuf` loads, and playing number patterns as pitch information
//! via a basic oscillator.
...
///
/// Each event in the generated script loads a sample into a `SndBuf`
/// and advances time (`=> now`).
///
/// # Examples
///
/// ```
...
pub fn export_sample_pattern_to_chuck(
    ...
    writeln!(file, "fun void play_samples() {{")?;
    ...
    writeln!(file, "}}")?;
    writeln!(file)?;
    writeln!(file, "spork ~ play_samples();")?;
    writeln!(file, "{total_time}::second => now;")?;
...
///
/// Number patterns are assumed to represent pitch (MIDI notes), mapped to an oscillator.
///
/// # Examples
///
/// ```
...
pub fn export_number_pattern_to_chuck(
    ...
    writeln!(file, "fun void play_notes() {{")?;
...
mod test_zero_cycle {
    ...
    fn export_sample_pattern_zero_cycles() {
        ...
        let module = eval_module(source, ReplMode::Loose).unwrap();
        let pattern = module.get("pattern").unwrap().as_sample_pattern().unwrap();
        let path = std::env::temp_dir().join("test_zero_sample.ck");
    ...
    fn export_number_pattern_zero_cycles() {
        ...
        let module = eval_module(source, ReplMode::Loose).unwrap();
        let pattern = module.get("pattern").unwrap().as_number_pattern().unwrap();
        let path = std::env::temp_dir().join("test_zero_number.ck");
