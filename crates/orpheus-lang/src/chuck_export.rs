...
//!
//! This exporter generates a strongly-timed `.ck` script containing sporked
//! shreds for evaluated Orpheus patterns, mapping standard Orpheus sample identifiers
//! to basic `SndBuf` loads, and playing number patterns as `SinOsc` pitch information.
...
pub fn export_sample_pattern_to_chuck(
    ...
    for event in events {
        ...
        let sp_sample = event.value.sample();
        let gain = event.value.gain();
        let rate = event.value.rate();

        writeln!(file, "    \"{sp_sample}.wav\" => buf.read;")?;
...
mod tests {
    ...
    fn chuck_exporter_generates_number_pattern() {
        ...
        assert!(content.contains("261.626 => osc.freq;")); // ~C4 (60)
    }
