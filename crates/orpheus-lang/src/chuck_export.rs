...
//!
//! This exporter generates a ``.ck`` script containing time advancements
//! for evaluated Orpheus patterns, triggering a default `SinOsc` for notes
//! and a `SndBuf` for samples.
...
///
/// Each event translates into a time advancement and a `SndBuf` trigger.
///
/// # Examples
///
/// ```
...
///
/// Number patterns are assumed to represent pitch, and map to a default `SinOsc`
/// in `ChucK`.
///
/// # Examples
///
...
pub fn export_number_pattern_to_chuck(
    ...
    for event in events {
        ...
        if start_time > current_time {
            let sleep_dur = start_time - current_time;
            writeln!(file, "0.0 => osc.gain;")?; // Silence while waiting
            writeln!(file, "{sleep_dur:.3}::second => now;")?;
        }

        let pitch = event.value;
        writeln!(file, "{pitch:.3} => Std.mtof => osc.freq;")?;
        writeln!(file, "0.5 => osc.gain;")?; // Note on
        writeln!(file, "{duration:.3}::second => now;")?; // Wait for note duration
...
mod tests {
    ...
    fn chuck_exporter_generates_number_pattern() {
        ...
        assert!(content.contains("60.000 => Std.mtof => osc.freq;"));
        assert!(content.contains("62.000 => Std.mtof => osc.freq;"));
        assert!(content.contains("64.000 => Std.mtof => osc.freq;"));
    }
