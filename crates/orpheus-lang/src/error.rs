...
mod tests {
    ...
    fn eval_error_from_try_from_int_error() {
        let try_err = u8::try_from(256_u16).unwrap_err();
        let err: EvalError = try_err.into();
    ...
    fn eval_error_from_fmt_error() {
        ...
        assert_eq!(err.to_string(), "an error occurred when formatting an argument");
    }

    #[test]
    fn eval_error_from_pattern_error() {
        let pat_err = orpheus_pattern::PatternError::InvalidDenominator { denominator: 0 };
