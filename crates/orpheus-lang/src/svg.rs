...
mod tests {
    ...
    fn export_cycle_count_zero_returns_error() {
        ...
        assert_eq!(
            super::export_sample_pattern_to_svg(pat, "test.svg", 0)
                .unwrap_err()
                .to_string(),
        ...
        assert_eq!(
            super::export_number_pattern_to_svg(pat, "test.svg", 0)
                .unwrap_err()
                .to_string(),
            "exporting requires at least one cycle"
        );
...
mod test_zero_cycle {
    ...
    fn export_sample_pattern_zero_cycles() {
        ...
        let module = eval_module(source, ReplMode::Loose).unwrap();
        let pattern = module.get("pattern").unwrap().as_sample_pattern().unwrap();
        let path = std::env::temp_dir().join("test_zero_sample.svg");

        let err = export_sample_pattern_to_svg(pattern, &path, 0).unwrap_err();
    ...
    fn export_number_pattern_zero_cycles() {
        ...
        let module = eval_module(source, ReplMode::Loose).unwrap();
        let pattern = module.get("pattern").unwrap().as_number_pattern().unwrap();
        let path = std::env::temp_dir().join("test_zero_number.svg");

        let err = export_number_pattern_to_svg(pattern, &path, 0).unwrap_err();
        assert_eq!(err.to_string(), "exporting requires at least one cycle");
    }
}
