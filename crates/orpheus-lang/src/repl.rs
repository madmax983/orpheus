...
pub fn run_stdio_with_engine_and_path(
    ...
    if let Some(msg) = warning {
        writeln!(stderr, "{} {}", "[Warn]".yellow().bold(), msg.as_str().yellow().bold())?;
    }

    if let Some(path) = startup_path {
        match session.open_file(path) {
            Ok(msg) => writeln!(stdout, "{} {}", "\u{2713}".green(), msg.as_str().green())?,
            Err(msg) => writeln!(stderr, "{} {}", "\u{2717}".red().bold(), msg.as_str().red().bold())?,
        }
    }
    ...
    E: Write,
{
    ...
    loop {
        ...
        match session.eval_line(trimmed) {
            Ok(message) => writeln!(stdout, "{} {}", "\u{2713}".green(), message.as_str().green())?,
            Err(message) => writeln!(stderr, "{} {}", "\u{2717}".red().bold(), message.as_str().red().bold())?,
        }
    }
