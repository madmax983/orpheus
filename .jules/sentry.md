...
## 2024-06-25 - Add Error From Tests
**Learning:** Evaluated code coverage and noticed missing tests for `From` conversions to `LoadError` for `TypeError` and `ParseError` and missing test cases for conversions to `EvalError` in `crates/orpheus-lang/src/error.rs`.
**Action:** Identify untested `From` or error mapping paths and add basic roundtrip tests to make sure error display logic isn't silently broken.
