...
## 2024-07-29 - [Unit tests for scheduler event translation]
**Learning:** Found that the `rational_to_frame_offset` logic handles critical bounds/conversions for logical clock to sample clock tracking, but it lacked edge-case verification for frame overflows, bounds correctness and duration clamping.
**Action:** Implemented targeted edge-case table-driven tests checking offset bounds and maximum clamping for durations within the `scheduler.rs` module.
