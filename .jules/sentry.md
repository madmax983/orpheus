...
## 2024-10-24 - repl & export test coverage gap
...
## 2024-10-24 - Coverage Gap in `eval.rs` Explicit-Time Parsing
...
## 2024-10-25 - [Test Coverage for Reverb DSP Filters]
**Learning:** Comb filters and allpass filters in the reverb DSP rely heavily on specific state-based transitions (e.g.  incrementation, feedback accumulation within a fixed buffer size). Assertions checking these states often require iterating past the buffer loop boundaries to observe actual feedback (buffer size 4 requires 5 process cycles). The  methods internally calculate an exact deterministic value that needs checking.
**Action:** Always write targeted tests for DSP algorithms testing their state boundary points (e.g., feedback points). Ensure  results (like from ) are ignored explicitly via  or completely evaluated to correctly increment state in tests.
## 2024-10-25 - [Test Coverage for Reverb DSP Filters]
**Learning:** Comb filters and allpass filters in the reverb DSP rely heavily on specific state-based transitions (e.g. index incrementation, feedback accumulation within a fixed buffer size). Assertions checking these states often require iterating past the buffer loop boundaries to observe actual feedback (buffer size 4 requires 5 process cycles). The process methods internally calculate an exact deterministic value that needs checking.
**Action:** Always write targeted tests for DSP algorithms testing their state boundary points (e.g., feedback points). Ensure unused_must_use results are ignored explicitly via `let _ = ...;` or completely evaluated to correctly increment state in tests.
