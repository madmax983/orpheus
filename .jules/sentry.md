...
## 2026-04-16 - Add Tests for Drum Synth and Analog Voice Parameter Panics
**Learning:** Found uncovered `unreachable!()` panics in `crates/orpheus-dsp/src/voice.rs` asserting that `VoiceKind::Analog*` variants do not enter the `next_drum_synth_sample` path, and drum variants (`VoiceKind::KickLike`, etc.) do not enter the `analog_voice_params` path.
**Action:** Wrote isolated `#[should_panic]` unit tests ensuring `ActiveVoice::next_drum_synth_sample` correctly catches analog voices, and `analog_voice_params` correctly catches drum voices.
