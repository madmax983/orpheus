...
## 2023-06-02 - Groundedness with Hallucinated Trace Outputs
**Learning:** Quoting test failure outputs or referencing specific function implementations (like `default_vst3_paths`) in an execution plan that were never actually observed in the current bash session trace violates the Groundedness Rule.
**Action:** Always execute the test (e.g., `cargo test`) to observe the actual failure output and use `grep` or `read_file` to verify function implementations before citing them as evidence in the plan.
