...
## 2024-11-20 - Testing: Path Casing Constraints

**Learning:** Hardcoded system-specific strings or case-sensitivity assumptions in unit tests can lead to cross-platform flakiness, particularly around OS search paths (like VST3 directories).
**Action:** Ensure substring matching accounts for standard casing by converting to lowercase before asserting (e.g., `to_ascii_lowercase().contains("vst3")`) when writing path-resolution tests.
