...
## 2024-05-27 - [Negative values during time/frame offset conversion]
**Learning:** Functions like `rational_to_frame_offset` that convert analytical time spans into sample offsets need to be robust to out-of-bounds evaluation queries with negative offsets, which can result in `None` due to underflow protections.
**Action:** When testing engine boundary checks, supply negative start times for sequence events to verify `None` and early returns are handled cleanly without affecting the running engine state.
