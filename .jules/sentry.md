...
## 2024-06-25 - Add EvalError::from tests
**Learning:** Evaluated code coverage and noticed missing tests for `From` conversions to `EvalError` for `std::io::Error`, `std::fmt::Error`, `std::num::ParseIntError`, `std::num::TryFromIntError` and `orpheus_pattern::PatternError`.
**Action:** Identify untested `From` or error mapping paths and add basic roundtrip tests to make sure error display logic isn't silently broken.
