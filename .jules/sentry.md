...
## 2026-07-12 - Fix missing zero-cycle unit tests for trackers
**Learning:** Found that `test_zero_cycle` sub-modules were entirely missing in `tracker.rs`, `abc_export.rs`, `arduino_export.rs`, `export.rs`, `gcode_export.rs`, `guitar_tab_export.rs`, `lilypond_export.rs`, `lua_export.rs`, and `supercollider_export.rs`. Added tests to cover these. Also removed some uses of unwrap where unecessary to maintain stability. Added tests for load error parsing errors.
**Action:** Ensure that all tests are added to verify error handling for specific cases across modules.
