...
## 2025-05-28 - [Division by Zero in every and when]
**Learning:** Certain rhythm transforms (`every` and `when`) can receive arguments evaluating to zero (like an explicit `0` for period). This leads to division-by-zero or remainder-by-zero panics via `cycle.rem_euclid(period)` deep within the Rust standard library.
**Action:** Always write explicit tests for evaluate engines with `0` or boundary inputs for period or transformation counts to ensure the system gracefully returns `EvalError` instead of panicking.
