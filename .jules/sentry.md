...
## 2025-05-17 - [Case-sensitive file paths in cross platform tests]
**Learning:** Found test failure in `vst3_descriptor_uses_standard_os_search_paths` due to asserting that `VST3` exists in a generated path. This caused tests to fail on systems using lowercase extensions for paths, such as linux `/usr/lib/vst3`.
**Action:** When writing tests that assert substrings generated by dynamic paths like system directories, evaluate the path components using case insensitive matching like `.to_lowercase().contains("...")` to ensure tests run across multiple platforms.
