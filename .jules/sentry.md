...
## 2024-06-25 - Improve SclError test coverage
**Learning:** Found coverage gaps in the `SclError` display logic, error conversions to `EvalError`, and failure branches of `parse_scala_entry` and `parse_scala_source` in `crates/orpheus-lang/src/scl.rs`.
**Action:** Add targeted unit tests in a `#[cfg(test)] mod tests` block to cover error cases and ensure parsing logic fails safely on invalid inputs.
