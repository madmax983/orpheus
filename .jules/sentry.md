...
## 2024-05-30 - Fix VST3 path assertion for Linux
**Learning:** Checking for `.contains("VST3")` will fail on Linux because the standard VST3 path uses lowercase (e.g., `/usr/lib/vst3`).
**Action:** When testing cross-platform OS paths, convert the path to lowercase before asserting (e.g., `.to_string_lossy().to_ascii_lowercase().contains("vst3")`) to prevent flaky tests.
