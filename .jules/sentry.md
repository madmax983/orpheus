...
## 2024-06-25 - Add should_show_binding_legend tests
**Learning:** Found that `should_show_binding_legend` in `crates/orpheus-lang/src/tui/style.rs` lacked unit tests testing the edge cases of space and height conditions and only had doc tests.
**Action:** Add robust tests covering the boolean logic using `ReplSession::with_engine(EngineHandle::stub())` and `.eval_line()`.
