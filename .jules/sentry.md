...
## 2024-06-06 - [Testing Unreachable Code Paths]
**Learning:** When writing `#[should_panic]` tests to verify `unreachable!` macros or panics, the input data might hit an early return or guard clause (e.g., `if matches!(...) { return; }`) before ever reaching the intended panic point, causing the test to fail because it doesn't panic.
**Action:** Carefully trace the execution path of the target function to ensure the test inputs are crafted to bypass early safe returns and actually reach the targeted unreachable branch.
