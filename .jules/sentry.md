...
## 2026-08-02 - markov_cumulative_row
**Learning:** The `markov_cumulative_row` function uses `.expect()` safely because it explicitly checks for a total weight > 0.0 before finding the last positive weight, but this invariant wasn't explicitly tested for all edge cases.
**Action:** Add tests specifically targeting the `.expect()` assumptions (all zeros, negative weights) to ensure they fail gracefully via the earlier `Result::Err` check rather than panicking in the `.expect()` block.
