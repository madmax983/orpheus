...
## 2024-06-03 - [Extract Constant Number Rational Coverage]
**Learning:** Found a missing panic/error path test coverage for the `extract_constant_number_rational` function in `eval.rs`.
**Action:** Always verify `extract_...` functions have comprehensive error tests when refactoring or adding tests, as they handle potentially dynamic `Value` variants.
