...
## 2024-06-25 - [Plugin Hosting: Linux VST3 Paths]
**Learning:** Linux operating systems typically use lowercase `vst3` for standard plugin search paths, causing case-sensitive string assertions (e.g., checking only for uppercase `VST3`) to fail.
**Action:** When validating standard OS VST3 search paths in tests (e.g., `plugin_hosting.rs`), explicitly check for both uppercase `VST3` and lowercase `vst3` variants.
