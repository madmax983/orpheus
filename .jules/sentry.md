...
## 2024-06-25 - Fix Cross-Platform Search Path Assertion
**Learning:** Checking hardcoded substrings like "VST3" for case-sensitive equality fails on systems where the path might be normalized or cased differently (e.g. "vst3" vs "VST3").
**Action:** Always convert paths to lowercase via `.to_string_lossy().to_lowercase()` before asserting `.contains()` in standard OS search path tests to prevent flaky failures across environments.
