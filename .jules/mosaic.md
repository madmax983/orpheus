...
## YYYY-MM-DD - Formatting Output in orpheus-lang
**Learning:** Unexplained bindings in the REPL (like `PluginPatternValue`) default to an ugly generic error message.
**Action:** When adding new values to the REPL, implement `Explain` and route it properly in `session.rs` using `comfy_table` or similar UI features to create a dashboard-like feel.
