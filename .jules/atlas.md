...
**[Fix Leaky Abstractions via Re-exports]
**Tangle:** The `UiTransportState` enum and `transport_state` function were `pub` inside the private module `tui::style`, but were inadvertently exposed to the public API via a `pub use` re-export block in `crates/orpheus-lang/src/lib.rs`. Additionally, the plugin structs (`ReplPlugin`, `BindingsPlugin`, `TransportPlugin`) and the `SharedState` struct were `pub struct` inside private modules but were unnecessarily leaking their internal details.
**Blueprint:** To fix the leaky abstractions, `UiTransportState` and `transport_state` were converted to `pub(crate)` and removed from the public re-export block in `lib.rs`. `SharedState` and the plugin structs, along with their fields and methods, were changed to `pub(crate)` visibility to ensure strong encapsulation within the workspace.
