...
**[Enforce Internal Module Boundary with  vs ]
**Tangle:** The SHELL=/bin/bash
NVM_INC=/home/jules/.nvm/versions/node/v22.22.1/include/node
SUDO_GID=1001
TERM_PROGRAM_VERSION=3.4
TMUX=/tmp/tmux-1001/default,2398,0
JAVA_HOME=/usr/lib/jvm/java-21-openjdk-amd64
DOTNET_ROOT=/usr/lib/dotnet
SUDO_COMMAND=/usr/bin/bash -c echo "${BASHPID}"
tmux new-session -d -s 'default' -c /app && tmux set-option remain-on-exit on
SUDO_USER=jules
...
OLDPWD=/app
TERM_PROGRAM=tmux
_=/usr/bin/env module in `orpheus-lang/src/types/mod.rs` was declared as `pub mod`, leaking the internal type-checker abstractions to the public API where only specific structs like `TypeEnv` and `TypeScheme` are expected to be consumed via facade re-exports.
**Blueprint:** Modified `pub mod env;` to `mod env;`. This reinforces strong module boundaries and correctly encapsulates the language's inference engine implementation details, forcing external usage to go through the intended facade re-exports in `types/mod.rs`.
**[Enforce Internal Module Boundary with `mod` vs `pub mod`]
**Tangle:** The `env` module in `orpheus-lang/src/types/mod.rs` was declared as `pub mod`, leaking the internal type-checker abstractions to the public API where only specific structs like `TypeEnv` and `TypeScheme` are expected to be consumed via facade re-exports.
**Blueprint:** Modified `pub mod env;` to `mod env;`. This reinforces strong module boundaries and correctly encapsulates the language's inference engine implementation details, forcing external usage to go through the intended facade re-exports in `types/mod.rs`.
