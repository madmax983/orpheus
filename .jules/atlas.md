...
**Tangle:** The `env` module in `orpheus-lang/src/types/mod.rs` was declared as `pub mod`, leaking the internal type environment implementation details to the public API.
**Blueprint:** Changed the module declaration from `pub mod env;` to `mod env;` in `crates/orpheus-lang/src/types/mod.rs`. This strictly enforces module encapsulation by keeping the module internal, while allowing the necessary types (`TypeEnv` and `TypeScheme`) to remain publicly accessible via the existing `pub use` statements.
