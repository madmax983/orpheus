...
**[Hide Internal Type Checker Abstractions]
**Tangle:** The `TypeEnv` and `TypeScheme` structs were publicly exported, unnecessarily leaking internal type-checker abstractions to the public API where only `TypedModule` is expected to be consumed.
**Blueprint:** Changed `pub mod env;` to `pub(crate) mod env;` in `types/mod.rs`, changed the re-exports of `TypeEnv` and `TypeScheme` to `pub(crate) use` in `types/mod.rs`, and removed them from the public API in `lib.rs`.
