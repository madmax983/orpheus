...
**[Remove Leaky Abstractions from UI Transports]
**Tangle:** The `orpheus-lang` crate has leaky public abstractions such as `UiTransportState`, `binding_legend_item`, etc., leaked through `lib.rs` from `tui::style`.
**Blueprint:** Removed the explicit re-exports of `UiTransportState`, `binding_legend_item`, and other private formatting utilities in `lib.rs` to stop leaking private module details into the public workspace level API.
