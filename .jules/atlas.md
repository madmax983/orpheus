...
**Tangle:** The `env` module in `orpheus-lang/src/types/mod.rs` was declared as `pub mod`, unnecessarily leaking the internal Hindley-Milner type-checker abstractions to the public API where only `TypedModule` is expected to be consumed.
**Blueprint:** Modified `crates/orpheus-lang/src/types/mod.rs` to change `pub mod env;` to `mod env;`. This enforces strong structural boundaries and correctly encapsulates the language's inference engine implementation details.
