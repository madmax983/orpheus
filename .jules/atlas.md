...
**Tangle:** The `env` module in `orpheus-lang/src/types/mod.rs` was declared as `pub mod`, unnecessarily exposing its full internal contents to the rest of the crate when only `TypeEnv` and `TypeScheme` are needed.
**Blueprint:** Changed `pub mod env;` to `mod env;` to enforce strict encapsulation, relying on the explicit `pub use` re-exports for the required types.
