...
**[Enforce Private Env Module]
**Tangle:** The `env` module inside `orpheus-lang/src/types/mod.rs` was declared as `pub mod env`, improperly leaking internal implementations like polymorphic `TypeScheme` creation, inference `insert`, and built-ins installation algorithms to external consumers who only need the `TypedModule`.
**Blueprint:** Modified `pub mod env;` to `mod env;` in `crates/orpheus-lang/src/types/mod.rs`. This enforces a strong boundary, hiding internal components while explicitly re-exporting necessary items via `pub use env::TypeEnv;` and `pub use env::TypeScheme;` to satisfy external dependencies.
