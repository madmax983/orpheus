...
**Tangle:** The `env` module in `orpheus-lang/src/types/mod.rs` was declared as `pub mod`, leaking the internal type-checker environment abstractions to the public API.
**Blueprint:** Changed `pub mod env;` to `mod env;` in `crates/orpheus-lang/src/types/mod.rs`. This enforces strong module boundaries by keeping the module internal while the intended public API (`TypeEnv` and `TypeScheme`) are explicitly exposed via `pub use`.
