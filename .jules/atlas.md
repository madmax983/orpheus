...
## YYYY-MM-DD - Encapsulate Hindley-Milner Type Environments
**Tangle:** The `TypeEnv` and `TypeScheme` structs in `orpheus-lang/src/types/env.rs` were public, unnecessarily leaking internal type-checker abstractions to the public API where only `TypedModule` should be exposed.
**Blueprint:** Downgraded the visibility of `TypeEnv` and `TypeScheme` to `pub(crate)` and removed their re-exports from `lib.rs` and `types/mod.rs` to enforce strong encapsulation.
