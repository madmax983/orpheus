...
**Tangle:** The `env` module in `orpheus-lang/src/types/mod.rs` was declared as `pub(crate) mod env;`, triggering `clippy::redundant_pub_crate` since the `types` module itself is private.
**Blueprint:** Removed `pub(crate)` and changed it to `mod env;`. The types `TypeEnv` and `TypeScheme` remain explicitly exposed through `pub use` in the `types/mod.rs` module, keeping encapsulation intact while satisfying clippy.
