...
**Tangle:** The `env` module inside `crates/orpheus-lang/src/types/mod.rs` was declared as `pub mod env;`, leaking its contents unnecessarily.
**Blueprint:** Modified `pub mod env;` to `mod env;` to strictly enforce the private module boundary. The intended public API (`TypeEnv` and `TypeScheme`) remains explicitly exposed via `pub use env::{TypeEnv, TypeScheme}`.
