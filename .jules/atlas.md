...
**[Clean up unused module exports]
**Tangle:** The `env` module inside `crates/orpheus-lang/src/types/mod.rs` was exported as `pub mod env;`, causing a clippy::redundant_pub_crate on the `TypeScheme` and `TypeEnv` exports inside the module.
**Blueprint:** Replaced `pub mod env;` with `mod env;`. Exported the structs explicitly using `pub use env::TypeEnv;` and `pub use env::TypeScheme;` to maintain the boundary.
