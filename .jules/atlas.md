...
**Tangle:** The `env` module in `orpheus-lang/src/types/mod.rs` was declared as `pub mod`, leaking the internal implementation details of the Hindley-Milner type environment to the public API, even though the structs `TypeEnv` and `TypeScheme` were already correctly re-exported via `pub use`.
**Blueprint:** Changed `pub mod env;` to `mod env;` in `crates/orpheus-lang/src/types/mod.rs`. This enforces strong module boundaries by keeping the module internal while the intended public APIs (`TypeEnv` and `TypeScheme`) are explicitly exposed via `pub use`.
