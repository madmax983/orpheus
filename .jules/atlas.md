...
**Tangle:** The `env` module inside `crates/orpheus-lang/src/types/mod.rs` was declared as `pub mod env;`, leaking internal type-checker implementations and causing a leaky abstraction.
**Blueprint:** Modified `pub mod env;` to `mod env;` in `crates/orpheus-lang/src/types/mod.rs` to enforce strong module boundaries while allowing `TypeEnv` and `TypeScheme` to still be exported properly via explicit `pub use` statements.
