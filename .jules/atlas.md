...
**[Fix Leaky Abstractions and Broken Intra-doc Links]
**Tangle:** The public documentation in several locations (`orpheus-lang/src/orca/mod.rs`, `orpheus-lang/src/orca/publish.rs`, `orpheus-lang/src/tui/style.rs`, `orpheus-dsp/src/engine.rs`, `orpheus-dsp/src/graph/genesis_psg.rs`, and `orpheus-dsp/src/graph/primitives.rs`) linked to private internal items (`publish`, `sample_trigger_from_event`, `METER_DECAY_HALF_LIFE_SECS`, `tests::lfsr_sequences_match_genesoxide`, `PSG_HEADROOM_SCALE`, `EnvCore`) and an unresolved `TransportSnapshot`, causing `rustdoc::private_intra_doc_links` warnings and exposing internal implementation details.
**Blueprint:** Replaced intra-doc links to private items with standard inline code (`` `item` ``), and fully qualified the unresolved public link to `[`orpheus_dsp::TransportSnapshot`]`.
