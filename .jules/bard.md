...
## 2026-08-05 - [Missing API and Broken Links in Documentation]
**Confusion:** The `cargo doc` generation failed due to several warnings for broken intra-doc links to private items without `--document-private-items` and missing `missing_docs` warnings across `orpheus-dsp` and `orpheus-lang`. This breaks the Bard philosophy of zero undocumented items and helpful error-free documentation builds.
**Clarification:** Updated broken intra-doc links like `[`PSG_HEADROOM_SCALE`]` to inline backticks ``PSG_HEADROOM_SCALE`` to preserve encapsulation while fixing the warning. Added missing module, function, and variant documentation across `scheduler.rs`, `sample_bank.rs`, `builtins.rs`, `loader.rs`, `pedal.rs`, `types/env.rs`, and `value.rs` ensuring all missing docs are handled.
## 2026-08-05 - [Missing API and Broken Links in Documentation]
**Confusion:** The `cargo doc` generation failed due to several warnings for broken intra-doc links to private items without `--document-private-items` and missing `missing_docs` warnings across `orpheus-dsp` and `orpheus-lang`. Also, failed to include examples for public items. This breaks the Bard philosophy of zero undocumented items and helpful error-free documentation builds with examples.
**Clarification:** Updated broken intra-doc links like `[`PSG_HEADROOM_SCALE`]` to inline backticks ``PSG_HEADROOM_SCALE`` to preserve encapsulation while fixing the warning. Added missing module, function, and variant documentation across `scheduler.rs`, `sample_bank.rs`, `builtins.rs`, `loader.rs`, `pedal.rs`, `types/env.rs`, and `value.rs` ensuring all missing docs are handled. Provided doc examples for `BuiltinFn::new`, `BuiltinFn::with_site_salt`, `ValidatedPedalNode::new`, `ValidatedPedalBinding::new`, and `TypeScheme::monomorphic`.
