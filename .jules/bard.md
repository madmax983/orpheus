...
## 2024-05-19 - [Fixing Broken Intra-Doc Links in Graph and Lang Modules]
**Confusion:** `cargo doc` warnings highlighted several broken intra-doc links caused by linking to private items (`METER_DECAY_HALF_LIFE_SECS`, `PSG_HEADROOM_SCALE`, `EnvCore`, `publish`, `sample_trigger_from_event`), non-existent test scope paths (`tests::volume_table_matches_2db_formula`), and unqualified external items (`TransportSnapshot` instead of `orpheus_dsp::TransportSnapshot`). It also attempted to link to `poll_orca` which does not exist in `ReplSession`.
**Clarification:** Fixed all broken and private intra-doc links across `orpheus-dsp` and `orpheus-lang`. Removed brackets from private items and test function links so they render as inline code, preserving documentation without violating `rustdoc::private_intra_doc_links`. Fully qualified `orpheus_dsp::TransportSnapshot`. Directed the `orca` link to `OrcaPublisher::poll` instead of the private `publish` module. Replaced `Self::poll_orca` with an explicit reference to `OrcaPublisher::poll` since `poll_orca` does not exist on `ReplSession`.
