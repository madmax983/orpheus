...
## 2024-05-19 - [Missing Value Enum Variant Documentation]
**Confusion:** The massive `BuiltinKind` enum in `value.rs` had dozens of variants missing documentation, leading to `missing_docs` warnings and obfuscated API context for language users. Other parts of the code were also undocumented.
**Clarification:** Added detailed `///` documentation to all missing variants and missing items describing their musical function rather than just repeating their names.
