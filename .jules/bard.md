...
## 2024-05-26 - [Missing BuiltinKind and AST Documentation]
**Confusion:** Many core components like `BuiltinKind` variants, AST and internal loading structs (`StrictLoadedFile`, `ValidatedPedalNode`), and polymorphic environments (`TypeScheme`) lacked documentation. This caused `missing_docs` warnings and prevented developers from understanding the "why" of these language primitives. Additionally, a test assertion assuming strict case sensitivity for plugin paths led to a failure on Linux machines where `/usr/lib/vst3` is standard.
**Clarification:** Added narrative `///` explanations for each `BuiltinKind` variant detailing their musical impact (e.g., `Euclid`, `Strum`, `Roll`). Documented struct fields for `StrictLoadedFile` and `TypeScheme` and applied `#[doc(hidden)]` to intentionally internal constructors to suppress noise. Updated `vst3_descriptor_uses_standard_os_search_paths` to check for paths in a case-insensitive manner (e.g. converting to lowercase) to eliminate the flaky test.
