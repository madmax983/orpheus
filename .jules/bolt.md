...
**Learning:** The `clippy::items_after_statements` lint (implied by `-D warnings` in newer Rust versions) forbids placing items, such as `use` imports, after any variable bindings or expressions within a block.
**Action:** Always place block-local `use` statements (like `use std::fmt::Write as _;`) at the very top of the function or block, before any variable declarations or operations, to prevent compilation errors.
