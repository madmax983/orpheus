...
**[Eliminate dynamic vector resizing on hot UI loop]**
**Learning:** Re-allocating new vectors sequentially (like `let mut spans = Vec::new();` per row) adds measurable overhead if the column count is predictable.
**Action:** Replace `Vec::new()` with `Vec::with_capacity(...)` when building rows or spans to eliminate dynamic vector resizing overhead on the hot UI render loop.
