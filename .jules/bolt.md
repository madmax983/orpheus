...
**[SmallVec Allocation Refactoring]**
**Learning:** `Vec::with_capacity()` still forces expensive heap allocations for dynamically constructed arrays of references (e.g. `Vec<&[f32]>` and `Vec<&mut [f32]>`) built dynamically in hot DSP loops. We can avoid all heap allocation by using `smallvec::SmallVec<[_; 32]>`. Because `SmallVec` does not implement `TrustedLen` optimally when `collect()` is called, we replaced `.iter().map(...).collect()` patterns with `.collect::<SmallVec<[_; 32]>>()` to guarantee inline stack arrays are used when DSP tracks are less than 32 (which is always true for the Orpheus audio graph context right now).
**Action:** Replace intermediate `Vec<&[T]>` collections inside `process` implementations with `SmallVec<[&[T]; 32]>`. If you get a borrow-checker error like "borrowed as mutable", it means the SmallVec outlives the immutable borrow later in the function block, just add an explicit `drop(variable)` immediately when the mutable lifetime is finished.
