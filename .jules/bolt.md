...
**[Pre-allocate Fixed Size UI Spans]**
**Learning:** In the TUI rendering loop, re-allocating new vectors sequentially (like `let mut spans = Vec::new();` per row) adds measurable overhead if the column count is predictable.
**Action:** Replace `Vec::new()` with `Vec::with_capacity(...)` when building rows or spans to eliminate dynamic vector resizing overhead on the hot UI render loop. Also, replace `format!("{}{}", col.content, " ".repeat(padding))` with `format!("{:<width$}", col.content, width = col_widths[i])` to avoid intermediate string allocations.
