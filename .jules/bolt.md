...
**Learning:** Chaining `.collect::<Vec<_>>().join(...)` creates a temporary, short-lived heap allocation that degrades performance, especially in hot paths or tight loops.
**Action:** Pre-allocate a `String` using `String::with_capacity()` and manually append to it using a loop and `push_str` (or `write!`) to eliminate the unnecessary allocation.
