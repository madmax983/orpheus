...
**[Tracker Exporter Allocation Optimization]**
**Learning:** Generating string slices inside inner loops of grid-formatting paths causes severe string allocations and cloned variables on hot paths when running exporters.
**Action:** Replaced dynamic `Vec<Option<String>>` strings with a flat byte-grid `Vec<Vec<u8>>` and pre-formatted string lookup arrays to prevent deep heap string churning on huge cycle generations.
