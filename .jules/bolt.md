...
**[AST BTreeMap Cloning Bottlenecks]**
**Learning:** Wrapping purely immutable data, such as a `BTreeMap` used for AST context mapping (`expr_site_salts`), inside an `Arc` avoids heavy O(N) heap allocations during hot paths (like closure creation or frequent evaluator scoping), reducing the operation to an O(1) atomic reference count increment. Thanks to `Arc` implementing `Deref`, existing read-only accesses do not need to be refactored.
**Action:** Identify heavily cloned, immutable maps in AST structures and wrap them in `Arc<T>` to dramatically improve performance without altering logic.

**[Cross-Platform Path Assertions]**
**Learning:** Hardcoding case-sensitive strings (e.g., `"VST3"`) in tests that assert on system resource paths causes failures across different operating systems due to platform-specific capitalization conventions.
**Action:** Always convert paths to lowercase (e.g., `path.to_string_lossy().to_lowercase().contains("vst3")`) when asserting on cross-platform directory names or extensions.
