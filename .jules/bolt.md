...
**[Safe DSP Slice Referencing]**
**Learning:** Bypassing the borrow checker with `unsafe` pointer transmutation (`std::mem::transmute` on `*mut [f32]`) to cache mutable slice references across audio frames introduces Undefined Behavior and violates strict aliasing.
**Action:** To eliminate per-frame `Vec` allocations safely, collect references into a stack-allocated `smallvec::SmallVec` directly within the `process()` function. Wrap the mutable collection and its dependent `.process()` call in an inner block scope `{ ... }` to ensure mutable borrows drop before any subsequent immutable slice borrows occur.
