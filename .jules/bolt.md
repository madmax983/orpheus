...
**[Avoid full environment cloning in type inference]
**Learning:** Deep cloning the entire type environment (`BTreeMap` of type bindings) on every parameterized function inference causes significant allocation overhead.
**Action:** Use in-place mutations of the environment `BTreeMap` by keeping track of shadowed bindings, inserting the new parameters before inference, and accurately restoring or removing them after inference. This avoids the heavy deep copy and still preserves lexical scoping correctness.
