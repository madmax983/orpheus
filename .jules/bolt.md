...
**[Rust: std::fmt::Write Error Handling]**\n**Learning:** When using `std::fmt::Write` macro (e.g., `write!(...)`) to append to a `String`, it returns a `std::fmt::Result` which often cannot be propagated with `?` if the enclosing function returns a different error type like `std::io::Result`.\n**Action:** Since formatting into an in-memory `String` cannot fail gracefully (it panics on OOM), use `.expect("formatting to a String cannot fail")` instead of `?` to handle the return value.
