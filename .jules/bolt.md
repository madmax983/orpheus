...
**[TUI String Padding Performance]**
**Learning:** Using `" ".repeat(padding)` inside a `format!` macro creates unnecessary intermediate `String` allocations.
**Action:** Use Rust's native format specifiers (`format!("{:<width$}", content, width = ...)`) to pad strings efficiently without temporary allocations.
