...
**[Optimizing Type Inference AST Traversal]**
**Learning:** Type inference and AST traversal frequently required deep cloning of recursive `Type` variants (like `Pattern` or `Function`) which triggered deep N^2 allocations just to follow substitutions or verify types recursively.
**Action:** Implemented `resolve_ref<'a>(&'a self, ty: &'a Type) -> Cow<'a, Type>` instead of cloning. This defers memory allocation down to only the rare case where a recursive type internally modifies its bindings. `Cow::Borrowed` prevents cloning in the common case.
