...
**[Replacing .collect::<Vec<_>>()]
**Learning:** We can replace iterator `.collect::<Vec<_>>()` calls with `Vec::with_capacity` and an explicit `for` loop pushing to the `Vec` to eliminate redundant iterator collection overhead on hot paths, especially across core session loops.
**Action:** Replace `collect::<Vec<_>>()` where memory and performance are a concern, with carefully initialized `Vec`s.
