...
**[Tracker Export Grid Optimization]**
**Learning:** Initializing large grids (like tracker visualizer grids) with `Option<String>` and allocating a new string per triggered step generates massive heap allocation overhead on export paths.
**Action:** Replace `Vec<Vec<Option<String>>>` grids with primitive state identifiers (like `Vec<Vec<u8>>`), and pre-compute any required formatted display strings exactly once before writing output rows.
