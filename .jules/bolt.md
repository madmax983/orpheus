...
**[Remove unnecessary type resolution allocations]**
**Learning:** Found unnecessary `Type` allocations during Hindley-Milner inference caused by calling `.clone()` instead of passing by reference.
**Action:** Changed `resolve` to take `&Type` instead of `Type` to eliminate `ty.clone()` overhead on hot type inference paths.
