...
**[Eliminating Intermediate String Allocations II]**
**Learning:** Chaining `.cloned().collect::<Vec<_>>().join(...)` causes unnecessary `Vec` and `String` allocations.
**Action:** Pre-allocate a single `String` with `.with_capacity()` and use `write!` from `std::fmt::Write` to build the string in place.
**Action:** Place `use std::fmt::Write;` at the beginning of the scope to avoid `clippy::items_after_statements` warnings.
**Action:** Use `{key}` directly in the macro `write!(&mut string, "{key}")` instead of `"{}"` to satisfy `clippy::uninlined_format_args`.
