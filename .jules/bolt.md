...
**[BTreeSet Allocation Optimization during Type Inference]**
**Learning:** Recursively creating and merging `BTreeSet` instances during AST type tree traversal (e.g., finding free type variables in `free_type_vars`) causes massive O(N) intermediate heap allocations on hot paths, especially when evaluating complex module environments. Using `.extend()` creates unnecessary intermediate sets that are immediately dropped.
**Action:** Replace functions that return collections recursively with a pattern that takes a mutable reference to a single collection (e.g., `&mut BTreeSet`), passing it down the call stack to accumulate values directly. This eliminates intermediate allocations and significantly reduces memory overhead.
