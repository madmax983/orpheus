...
**[Remove recursive tree clone overhead]
**Learning:** Calling `self.resolve(ty.clone())` inside the recursive `occurs` check for Hindley-Milner type inference caused an explosion of allocations, leading to a massive performance penalty.
**Action:** Instead of deeply cloning the AST tree (Type) at every node traverse, refactor `occurs` to look up variables directly from `self.substitutions` by reference, completely bypassing the `resolve` step and its associated allocations during the occurs check.
