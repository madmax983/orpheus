...
**[Optimize free_type_vars]
**Learning:** `free_type_vars` created, merged, and discarded many `BTreeSet` instances during Hindley-Milner type inference, heavily allocating in the hot-path `generalize` and `free_vars_in_env` methods.
**Action:** Extracted `free_type_vars` logic into a `free_type_vars_into` function that accumulates type variables into a single mutable `BTreeSet` reference.
