...
## 2026-04-14 - Optimize sample list format collection\n**Learning:** Constructing intermediate `Vec<String>`s just to join strings is an unnecessary double allocation penalty, especially when elements can just borrow `&str` from the original `Event` tree.\n**Action:** Use pre-allocated Strings and sequentially `push_str()` borrowed references, taking advantage of `size_hint()` or element counts to bound the capacity and avoid re-allocations.
