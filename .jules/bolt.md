...
**[String Arc Allocation]**
**Learning:** When converting a `String` inside an `Expr::String(value)` to an `Arc<str>` via `Value::String(value.clone().into())`, it unnecessarily allocates an intermediate `String` copy before converting it to an `Arc<str>`.
**Action:** Use `value.as_str().into()` to directly allocate the byte slice into the `Arc<str>`, completely avoiding the intermediate `String` allocation.
