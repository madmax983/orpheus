...
## 2024-05-21 - [Cow for Mixed String Collections]
**Learning:** Constructing collections like `Vec<Option<String>>` in a loop where some items are dynamically formatted and others are repeated static strings (e.g., continuation marks) causes excessive heap allocations.
**Action:** Use `std::borrow::Cow<'_, str>` within the collection to wrap dynamically formatted strings in `Cow::Owned` and repeated static strings in `Cow::Borrowed` to minimize memory allocations on hot paths.
