...
**[Rust/Performance: Recursive Tree Traversal]**
**Learning:** When recursively traversing deeply nested enums (like AST nodes or type trees during inference or an `occurs` check), calling methods that take ownership and force deep cloning of the tree at every level (e.g., `resolve(ty.clone())`) introduces significant memory allocation overhead.
**Action:** Traverse by reference (`&Type`) and perform local variable substitutions manually to prevent quadratic memory allocation overhead on the hot path.
