...
**[String Building and Formatting in Loops]**
**Learning:** Accumulating strings inside a hot loop using `Vec::new()`, `params.push(format!(...))`, and `params.join()` causes unnecessary heap allocations.
**Action:** When writing to an I/O stream (e.g., `std::fs::File`), use `write!` and `writeln!` to format directly into the stream, and wrap the stream in a `std::io::BufWriter` to buffer writes, avoiding the string allocation entirely.
