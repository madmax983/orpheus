...
**[Session Reload Cloning Optimization]**
**Learning:** `midi_note_mappings.get(&event.note).cloned()` in `apply_midi_note_mappings` forced unnecessary heap allocations on strings because we were immediately indexing into another map which accepts string slices `&str` through `borrow::Borrow`. The same issue was present in `open_file` when parsing bindings and joining strings.
**Action:** Remove `.cloned()` and pass references directly to avoid unnecessary heap allocations during session processing.
