...
**Learning:** Identified duplicate code logic for plain text file initialization, header writing, and event looping in `crates/orpheus-lang/src/txt.rs` across sample and number pattern exports.
**Action:** Extract text export boilerplate into a generic `export_pattern_events_to_txt` helper, removing duplication and keeping file I/O operations central, similar to JSON and CSV exporting.
