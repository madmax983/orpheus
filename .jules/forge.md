...
**Learning:** The `apply_every` function contained a repetitive, verbose match block over `Value` returning `EvalError` for everything except `SamplePattern` and `NumberPattern`.
**Action:** Extracted the core routing logic into a `apply_pattern_transform` helper function, utilizing closures to safely extract mutable closures, and flattened the function.
